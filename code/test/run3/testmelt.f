c mgc - revisit Greuell and Konzelmann 1994 - they describe a method to partition
c sw penetration and how they deal with meltwater and grid discretization

c mgc run 3 - add column-variable ice density, use column average for spect_ext
...
c	  real :: ro_snow_z, ro_snow_z_old
c	  real ro_snow_z(JJ) !could be better to define in ICEINIT
...
c	  open (21,file='density.dat') ! added to ice2 instead
...
c mgc add ro_snow_z (removed for now)	 
...
c          print *,'Annual Loop Number =',kkk
          ...
          do iter=1,maxiter
c mgc fold is read in met data 
...
c mgc add ro_snow_z
     ...
     &        water_depth_old,water_flux,xLf,ro_snow_z)
...
c              write (20,*) k,T_old(k)-Tf,gamma(k),xmelt(k),water_frac(k)
     ...
     &          ro_snow_z(k)
c mgc write ro_snow_z, but maybe move to top so ro_snow_z that affects this
c timestep is written
c			  write (21,*) ro_snow_z(k)			  
c mgc add
...
c mgc hoffman computes 'totalheat0' from 1:JJ which is the sum of
c the sensible heat (Told - Tf) * Cpsnow * ro_snow * dy and the 
c latent heat: water_frac * ro_snow * xLf * dy
...
c mgc hoffman added a call to CONDUCT and checks if ICENERGY is 
c finished using ifinalcall = 1 and if so, adds up totalheat1
...
c mgc hoffman calls CONDUCT again and then does another totalheat2
...
c mgc hoffman does another CONDUCT and a totalheat3 that involves 'sourceterm'
...
c mgc hoffman issues warning if water_depth.gt.1.0
...
c I could also average over any depth (e.g. 1 m) to get a lower density
c and that would affect the spect_ext calculations  
	  ro_snow = 800 * (1 - water_depth/15.0) ! total column depth is 15.	  

c mgc add ro_snow_z, note this ro_snow_z is used to compute the
c extcoefs for the next timestep, so I may want to move the write
c command further up above the call to ICE_ENERGY
c water_frac is cumulative, so use 800 as baseline 	  
	  do j=1,JJ
		  ro_snow_z(j) = 800 * (1-water_frac(j))
	  enddo		  
...
c mgc add ro_snow_z ! leaving out for now
...
c mgc for this run (2) I am leaving ro_snow fixed and instead will
c just let ro_snow_z affect scattering
...
c mgc at minimum, I should let this water drain, perhaps before
c adding provision for threshold drainage
c mgc note this gets added to totalmelt at the next iteration (depth)		  
...
c mgc hoffman replaced Qsi with Qsip and added his 'internal_heating_corr'
...
c mgc hoffman sets Sc(j) = 0 for the 'top layers that contribute to SEB directly'
...
c mgc hoffman replaced Qsi with Qsip and added his 'internal_heating_corr',
c and sets Sc(j) = 0 for the 'top layers that contribute to SEB directly',
c as in GETNEWT
...
c mgc hoffman 'moved this into SFCTEMP'
c Compute the flux contribution due to conduction.
        CALL CONDUCT(icond_flag,Qc,gamma,T_old,dy_p,JJ)

c mgc hoffman added a bunch of the subsurface internal 
c varibles to the SFCTEMP call, check for clues
...
c mgc solar in is calculated here, then (I believe) scaled by the 
c spectrum in solar.dat. Hoffman used station data to correct it, not 
c sure why he wouldn't just add it to the input met data, maybe there
c is missing data, but 
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c mgc hoffman reads in station P data
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c mgc hoffman used the values over ice
...
c mgc add Chi here, and define Qmskin
c mgc hoffman made a bunch of changes, added qsfactor (Chi) and 
c calculates ablation from Qm (surface_melt)
...
c mgc hoffman added qsfactor*(1-albedo)
...
c mgc add qsfactor to EEE = 1-albedo ...
