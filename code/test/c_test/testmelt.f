...
c	  real ro_star(JJ) !could be better to define in ICEINIT
...
c mgc add ro_star (removed for now)	 
     ...
     &    n_snowgrain_radius,total_solar,ro_star)
...
c mgc add ro_star
     ...
     &        water_depth_old,water_flux,xLf,ro_star)
...
c mgc use this to debug 
            print *, iter,Tair-Tf,Tsfc-Tf,rh,windspd,
     &        Qsi,Qli,Qle,Qh,Qe,Qc,Qm,balance,albedo
c mgc end            
			
c Save the ice data.
            write (19,*) Tsfc-Tf,Qsi,Qsip,water_depth,water_flux
...
c mgc write ro_star, but maybe move to top so ro_star that affects this
...
c mgc add ro_star
     ...
     &    up,down,total_solar,xLs,Rv,ro_water,ro_star)
      endif
...
c mgc add ro_star ! leaving out for now
...
c just let ro_star affect scattering
...
c mgc add ro_star	  
     ...
     &  r_snow,qext,ss_coalb,g,spect_extcoef_snow,nclasses,ro_star)
...
c mgc PICK UP HERE ... realized at this point that by making ro_star
